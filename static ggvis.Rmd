...
***
###Polish a bit: addind axes and legends
...
###Customize the color with `scale`
* `scale_numeric()`
* `scale_datetime()`
* `scale_logical()`
* `scale_nominal()`
...
* `scale_numeric()` is contineuous color mapping to a range of color
```{r}
...
* `scale_nominal()` is categorical mapping of color
```{r}
...
###`scale` more than color
* `scale` can be used not only to color, but to other property, like `opacity` here
```{r}
